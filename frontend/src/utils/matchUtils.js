/**
 * HireNexa Deterministic Matching Engine (Client-side)
 * 
 ...
 * 
 * First Job Mode Scoring:
 * - Skills Match: 50%
 * - Hands-on Projects / Certifications / GitHub: 20%
 * - Experience: 10% (Entry <= 2 yrs receives full credit)
 * - Education: 10%
 * - Keywords / Coursework: 10%
 */
...
export function calculateJobMatch(candidate, job) {
    ...
    if (isFirstJobMode) {
        ...
        // Experience: 10% (Junior <= 2 yrs receive full credit)
        if (jobExp <= 2) {
            expScore = 10;
